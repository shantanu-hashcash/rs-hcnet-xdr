...
doc:
	...
	RUSTDOCFLAGS="--cfg docs" cargo +nightly doc --package hcnet-xdr --all-features $(CARGO_DOC_ARGS)
...
ifeq ($(LOCAL_XDRGEN),)
	docker run -i --rm -v $$PWD:/wd -w /wd docker.io/library/ruby:latest /bin/bash -c '\
		...
		gem specific_install https://github.com/hcnet/xdrgen.git -b $(XDRGEN_VERSION) && \
